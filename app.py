...
def transformar_link(link_original):
    try:
        ...
        if pa == '38':
            ...
            for param in ['redirect_status_position', 'qualification_term_question_id', 'qualification_term_question_key', 'matched_qouta_id', 'reason_id', 'trans_id', 'disqualify_reason']:
        ...
        nueva_url = urlunparse((parsed.scheme, parsed.netloc, parsed.path, parsed.params, nueva_query, parsed.fragment))
...
def init():
    try:
        ...
        if not admin:
            ...
            db.session.commit()
        return "✅ Base de datos lista y admin creado"
    except Exception as e:
